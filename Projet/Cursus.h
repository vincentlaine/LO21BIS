...
enum CategorieCursus {
    TC, Cursus, Branche, Filiere,HuTech, Mineur, autre, first=TC, last=autre
};
...
private:
    ...
    Curs(QString n, CategorieCursus cat, int nbCS, int nbTM, int nbTSH, int nbSP) : nom(n), categorie(cat), credCS(nbCS), credTM(nbTM), credTSH(nbTSH), credSP(nbSP),nbUV(0), nbMaxUV(4), tabUV(new QString[4]){}
